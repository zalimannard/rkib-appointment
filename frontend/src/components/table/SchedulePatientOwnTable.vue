  ...
  setup(props, {emit}) {
    ...
    const requestSchedule = async () => {
      try {
        ...
        let myPatientId = response.data.patient.id;
        ...
        try {
          ...
          schedules.value = response.data.filter((schedule: ScheduleResponse) => {
            return schedule.appointment != null &&
                schedule.appointment.patient.id === myPatientId;
          });
          onEditFilter();
        } catch (error) {
    ...
    function calcAppointmentPresentation(item: AppointmentResponse) {
      ...
      return (item.doctorNote != null ? item.doctorNote : "")
          + " / Статус: " + item.status.name;
    }

    function calcDate(item: ScheduleResponse) {
      return utils.fromIsoToDefault(utils.fromDateTimeToIsoDate(item.appointmentTime))
    }
